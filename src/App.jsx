...
const BookCover = ({ book, style = {} }) => {
  ...
  // Always show placeholder for now (cover images coming soon)
  return (
    <div style={{
...
// ─── Main App ───
export default function ReadersRealm() {
  ...
  const [bookClickCount, setBookClickCount] = useState(0);
  ...
  const [shelves, setShelves] = useState({
    ...
    "Kid #1 Reading List": [], "Books to Buy My Best Friend": [],
  });
  ...
  const handleBookClick = useCallback((book) => {
    ...
    const n = bookClickCount + 1;
    setBookClickCount(n);
    if (n >= 2 && !surveyShown) {
      setTimeout(() => { setShowSurveyPopup(true); setSurveyShown(true); }, 3000);
    }
  }, [bookClickCount, surveyShown]);
  ...
  const handleSignUp = async () => {
    ...
    setSignUpError("");
    try {
      const res = await fetch(SIGNUP_SCRIPT_URL, {
        method: "POST",
        mode: "no-cors",
      ...
      setSignUpDone(true);
      setTimeout(() => { setIsSignedIn(true); setShowSignUp(false); setSignUpDone(false); setSignUpEmail(""); }, 1500);
    } catch (err) {
  ...
  const FiltersContent = ({ isMobile = false }) => (
    <div style={{ padding: isMobile ? "20px" : 0 }}>
      ...
      <CollapsibleSection title="Sort By" defaultOpen><Dropdown value={sortBy} onChange={setSortBy} options={["New", "Popular", "Title", "Author"]} /></CollapsibleSection>
      <CollapsibleSection title="Genre" defaultOpen><MultiSelect options={ALL_GENRES} selected={selectedGenres} onChange={setSelectedGenres} placeholder="Select genres..." /></CollapsibleSection>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
          {CONTENT_RATINGS.map(r => (
            <label key={r} style={{ display: "flex", alignItems: "center", gap: "8px", cursor: "pointer", fontSize: "13px", color: C.cream }}>
              <input type="checkbox" checked={contentRatings.includes(r)} onChange={() => setContentRatings(p => p.includes(r) ? p.filter(x => x !== r) : [...p, r])} style={{ accentColor: C.copper }} />{r}
            </label>
          ))}
        </div>
      </CollapsibleSection>
      <CollapsibleSection title="Include" color={C.teal}>
        <MultiSelect label="Warnings" options={ALL_WARNINGS} selected={includeWarnings} onChange={setIncludeWarnings} placeholder="Include warnings..." />
        <MultiSelect label="Tropes" options={ALL_TROPES} selected={includeTropes} onChange={setIncludeTropes} placeholder="Include tropes..." />
        <MultiSelect label="Tags" options={ALL_TAGS} selected={includeTags} onChange={setIncludeTags} placeholder="Include tags..." />
      </CollapsibleSection>
      <CollapsibleSection title="Exclude" color="#8B3A3A">
        <MultiSelect label="Warnings" options={ALL_WARNINGS} selected={excludeWarnings} onChange={setExcludeWarnings} placeholder="Exclude warnings..." />
        <MultiSelect label="Tropes" options={ALL_TROPES} selected={excludeTropes} onChange={setExcludeTropes} placeholder="Exclude tropes..." />
        <MultiSelect label="Tags" options={ALL_TAGS} selected={excludeTags} onChange={setExcludeTags} placeholder="Exclude tags..." />
      </CollapsibleSection>
      <CollapsibleSection title="More Options" color={C.teal}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "4px", marginBottom: "12px" }}>
          {["Completed", "Ongoing", "Standalone", "Any"].map(s => (
            <label key={s} style={{ display: "flex", alignItems: "center", gap: "8px", cursor: "pointer", fontSize: "13px", color: C.cream }}>
              <input type="radio" name="series" checked={seriesStatus === s} onChange={() => setSeriesStatus(s)} style={{ accentColor: C.copper }} />{s}
            </label>
          ))}
        </div>
        ...
        <Dropdown label="Language" value={language} onChange={setLanguage} options={ALL_LANGUAGES} placeholder="Any Language" />
      </CollapsibleSection>
            ...
            <div style={{ flex: 1, minWidth: "200px" }}>
              ...
              {isSignedIn && (
                <div style={{ marginTop: "12px", position: "relative" }}>
                  <button onClick={() => setShowShelfPicker(showShelfPicker === book.id ? null : book.id)} style={{ padding: "8px 16px", background: shelf ? C.teal : "rgba(194,122,58,0.2)", border: `1px solid ${shelf ? C.teal : C.copper}`, borderRadius: "6px", color: shelf ? "#fff" : C.copper, fontSize: "12px", fontWeight: 600, cursor: "pointer" }}>
                    {shelf ? `📚 ${shelf}` : "＋ Add to Shelf"}
                  </button>
                  {showShelfPicker === book.id && (
  ...
  return (
    <div style={{ fontFamily: "'Source Sans 3', 'Segoe UI', sans-serif", background: "linear-gradient(180deg, #1a1220 0%, #2B1E2F 100%)", minHeight: "100vh", color: C.cream }}>
      ...
      {/* Welcome Modal */}
      {showWelcome && (
        <div onClick={() => setShowWelcome(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", backdropFilter: "blur(6px)", zIndex: 2000, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
          <div onClick={e => e.stopPropagation()} style={{ background: "linear-gradient(160deg, #2B1E2F 0%, #1a1220 100%)", borderRadius: "20px", maxWidth: "480px", width: "100%", padding: "40px", textAlign: "center", border: "1px solid rgba(194,122,58,0.3)", boxShadow: "0 24px 80px rgba(0,0,0,0.6)" }}>
            <div style={{ fontSize: "20px", fontFamily: "'Playfair Display', serif", color: C.cream, marginBottom: "8px" }}>Welcome to <span style={{ color: C.copper }}>Readers' Realm!</span></div>
            <div style={{ fontSize: "28px", marginBottom: "16px" }}>🎉</div>
            <div style={{ fontSize: "14px", color: C.copper, fontWeight: 600, marginBottom: "8px" }}>Prototype Preview</div>
            <p style={{ color: "rgba(232,220,203,0.8)", fontSize: "13px", lineHeight: 1.7, margin: "0 0 12px" }}>
              You are currently viewing a prototype of an app we are building. Please be aware it has limited functions and may not yet be fully accurate. It is incomplete at this time, but we would love your input!
            </p>
            <div style={{ background: "rgba(53,96,90,0.15)", border: "1px solid rgba(53,96,90,0.3)", borderRadius: "8px", padding: "10px 14px", marginBottom: "16px" }}>
              <p style={{ color: "rgba(232,220,203,0.7)", fontSize: "12px", lineHeight: 1.6, margin: 0 }}>
                📚 <strong style={{ color: C.copper }}>Note:</strong> In the full version, books will display their actual cover images. This prototype currently shows placeholder covers — cover image support is coming soon!
              </p>
            </div>
            <p style={{ color: "rgba(232,220,203,0.8)", fontSize: "13px", lineHeight: 1.7, margin: "0 0 20px" }}>
              📋 Please consider filling out our <a href={SURVEY_URL} target="_blank" rel="noreferrer" style={{ color: C.teal, fontWeight: 600 }}>survey</a> — it will help immensely in deciding what features to add and addressing any complaints you may have.
            </p>
            <button onClick={() => setShowWelcome(false)} style={{ padding: "14px 40px", background: `linear-gradient(135deg, ${C.copper}, #A86830)`, color: "#fff", border: "none", borderRadius: "10px", fontSize: "15px", fontWeight: 700, cursor: "pointer", letterSpacing: "0.5px" }}>Explore the App</button>
            <div style={{ color: "rgba(232,220,203,0.5)", fontSize: "12px", marginTop: "16px" }}>Thank you for being an early visitor ✨</div>
          </div>
        </div>
      )}
      ...
      {/* Sign Up Modal */}
      {showSignUp && (
        <div onClick={() => setShowSignUp(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", backdropFilter: "blur(6px)", zIndex: 2000, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
          <div onClick={e => e.stopPropagation()} style={{ background: "linear-gradient(160deg, #2B1E2F 0%, #1a1220 100%)", borderRadius: "20px", maxWidth: "440px", width: "100%", padding: "36px", textAlign: "center", border: "1px solid rgba(194,122,58,0.3)" }}>
            <h2 style={{ fontFamily: "'Playfair Display', serif", color: C.cream, marginBottom: "12px" }}>Sign Up</h2>
            {signUpDone ? (
              <div><div style={{ fontSize: "40px", marginBottom: "12px" }}>✅</div><p style={{ color: C.cream, fontSize: "14px" }}>You're in! Loading your personalized experience...</p></div>
            ) : (
              <>
                <div style={{ background: "rgba(53,96,90,0.15)", border: "1px solid rgba(53,96,90,0.3)", borderRadius: "10px", padding: "14px", marginBottom: "20px", textAlign: "left" }}>
                  <p style={{ color: "rgba(232,220,203,0.85)", fontSize: "12px", lineHeight: 1.6, margin: 0 }}>
                    🧪 <strong style={{ color: C.copper }}>Beta Preview:</strong> Readers' Realm is still in beta stages. By signing up, you'll get to see what it would be like to use the site as a member. Your email will be stored so we can notify you when the website properly launches!
                  </p>
                </div>
                <input type="email" placeholder="Enter your email" value={signUpEmail} onChange={e => setSignUpEmail(e.target.value)} onKeyDown={e => e.key === "Enter" && handleSignUp()} style={{ width: "100%", padding: "12px 16px", background: "rgba(43,30,47,0.6)", border: "1px solid rgba(194,122,58,0.3)", borderRadius: "8px", color: C.cream, fontSize: "14px", marginBottom: "12px", boxSizing: "border-box" }} />
                <button onClick={handleSignUp} style={{ width: "100%", padding: "14px", background: `linear-gradient(135deg, ${C.copper}, #A86830)`, color: "#fff", border: "none", borderRadius: "10px", fontSize: "15px", fontWeight: 700, cursor: "pointer" }}>Sign Up for Beta</button>
                {signUpError && <div style={{ color: "#C27A3A", fontSize: "12px", marginTop: "8px" }}>{signUpError}</div>}
                <div style={{ color: "rgba(232,220,203,0.4)", fontSize: "11px", marginTop: "10px" }}>No password required — just your email</div>
              </>
            )}
          </div>
        </div>
      ...
      {/* Survey Popup */}
      {showSurveyPopup && (
        <div style={{ position: "fixed", bottom: "24px", right: "24px", zIndex: 1500, background: "linear-gradient(135deg, #2B1E2F, #1a1220)", borderRadius: "16px", padding: "20px 24px", maxWidth: "320px", border: "1px solid rgba(194,122,58,0.4)", boxShadow: "0 12px 40px rgba(0,0,0,0.5)", animation: "slideUp 0.4s ease-out" }}>
          <button onClick={() => setShowSurveyPopup(false)} style={{ position: "absolute", top: "8px", right: "12px", background: "none", border: "none", color: "rgba(232,220,203,0.5)", fontSize: "16px", cursor: "pointer" }}>✕</button>
          <p style={{ color: C.cream, fontSize: "13px", lineHeight: 1.6, margin: "0 0 12px" }}>If you like this app so far, fill out this survey so we know this idea is something people want!!</p>
          <a href={SURVEY_URL} target="_blank" rel="noreferrer" style={{ display: "inline-block", padding: "10px 20px", background: C.teal, color: "#fff", borderRadius: "8px", fontSize: "13px", fontWeight: 700, textDecoration: "none" }}>📋 Take the Survey</a>
        </div>
      )}
      ...
      <nav style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 24px", background: "rgba(74,44,35,0.5)", borderBottom: "1px solid rgba(194,122,58,0.2)", flexWrap: "wrap", gap: "10px", position: "sticky", top: 0, zIndex: 100, backdropFilter: "blur(12px)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px", flexShrink: 0 }}>
          <a href={SURVEY_URL} target="_blank" rel="noreferrer" style={{ padding: "8px 16px", background: "rgba(53,96,90,0.2)", border: `1px solid ${C.teal}`, borderRadius: "8px", color: C.teal, fontSize: "13px", fontWeight: 600, textDecoration: "none", display: "flex", alignItems: "center", gap: "6px" }}>📋 <span className="nav-label">Survey</span></a>
          {!isSignedIn ? (
            <button onClick={() => setShowSignUp(true)} style={{ padding: "8px 16px", background: `linear-gradient(135deg, ${C.copper}, #A86830)`, border: "none", borderRadius: "8px", color: "#fff", fontSize: "13px", fontWeight: 700, cursor: "pointer" }}>Sign Up</button>
          ) : (
            <button style={{ padding: "8px 16px", background: "rgba(194,122,58,0.2)", border: `1px solid ${C.copper}`, borderRadius: "8px", color: C.copper, fontSize: "13px", fontWeight: 600, cursor: "default" }}>👤 Member</button>
          )}
          <button onClick={() => setShowAbout(true)} style={{ padding: "8px 16px", background: "rgba(232,220,203,0.08)", border: "1px solid rgba(232,220,203,0.15)", borderRadius: "8px", color: C.cream, fontSize: "13px", fontWeight: 600, cursor: "pointer" }}>About</button>
        </div>
      </nav>
